...
const int HEART_BEAT_HISTORY_LENGTH = 100;
...
class sensorgraph {
    ...
    GLfloat xPos[HEART_BEAT_HISTORY_LENGTH];
    ...
    AccelerometerData sensorData[HEART_BEAT_HISTORY_LENGTH *2];
    AccelerometerData sensorDataFilter;
    int sensorDataIndex;

    //Heart Beat Data
    std::queue<float> heartRateDataFromBluetooth;

    float heartBeatData[HEART_BEAT_HISTORY_LENGTH *2];
    int indexOfHeartBeatData;
 ...
 public:
    ...
    void generateXPos() {
        for (auto i = 0; i < HEART_BEAT_HISTORY_LENGTH; i++) {
            float t = static_cast<float>(i) / static_cast<float>(HEART_BEAT_HISTORY_LENGTH - 1);
    ...
    void update() {
        ...
        /*
         * Used to obtain information about the accelerometer
         *
        ...
        sensorData[HEART_BEAT_HISTORY_LENGTH+sensorDataIndex] = sensorDataFilter;
        sensorDataIndex = (sensorDataIndex + 1) % HEART_BEAT_HISTORY_LENGTH;
        */

        for (int j = 0; j < HEART_BEAT_HISTORY_LENGTH*2 && !heartRateDataFromBluetooth.empty(); ++j) {
            heartBeatData[j] = heartRateDataFromBluetooth.front();
            heartRateDataFromBluetooth.pop();
        }
        indexOfHeartBeatData = (indexOfHeartBeatData + 1) % 100;
    }
    ...
    void render() {
        ...
        //Draw the array for the X Movement
        glEnableVertexAttribArray(vSensorValueHandle);
        glVertexAttribPointer(vSensorValueHandle, 1, GL_FLOAT, GL_FALSE, sizeof(float),
                              &heartBeatData[indexOfHeartBeatData]);

        glUniform4f(uFragColorHandle, 1.0f, 1.0f, 0.0f, 1.0f);
        glDrawArrays(GL_LINE_STRIP, 0, HEART_BEAT_HISTORY_LENGTH);


        //Draw the array for the Y Movement
        ...
        //Draw the array for the Z Movement
        //glVertexAttribPointer(vSensorValueHandle, 1, GL_FLOAT, GL_FALSE, sizeof(AccelerometerData),
    ...
    void addHeartRateDataFromBluetooth(float data){
        heartRateDataFromBluetooth.push(data);
    }
};
...
JNIEXPORT void JNICALL
    ...
    JNIEXPORT void JNICALL
    Java_com_android_sensorgraph_SensorGraphJNI_addHeartRateDataFromBluetooth(JNIEnv *env, jclass type, jfloat data) {
        (void)env;
        (void)type;
        gSensorGraph.addHeartRateDataFromBluetooth(data);
    }
}
